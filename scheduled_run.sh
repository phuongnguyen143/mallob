...
WORK_YOURPROJECTNAME="worstlbd-part1"
...
jwl=1000
numInstances=100
scheduleparameter="-c=1 -ajpc=1 -job-desc-template=not_very_hard_instances_2022_part1.txt -job-template=templates/job-template.json -client-template=templates/client-template.json -J=$numInstances -jwl=$jwl -pls=0"
...
malloboptions="-t=4 -T=$timeout -sleep=1000 -appmode=fork -v=3 -interface-fs=0 -trace-dir=. -processes-per-host=$NPROCS -regular-process-allocation=1 -max-lits-per-thread=50000000 -strict-clause-length-limit=20 -clause-filter-clear-interval=$cfci -nccts=$nccts -scfci=$scfci -max-lbd-partition-size=2 -export-chunks=20 -clause-buffer-discount=$cbdf -satsolver=$portfolio -ccb=$ccb -spo=$spo -lbdm=$lbdm"
