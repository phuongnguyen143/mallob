...
void SharingManager::onProduceClause(int solverId, int solverRevision, const Clause &clause, int condVarOrZero)
{
	...
	if (_solver_revisions[solverId] != solverRevision)
		return;
	...
	if (condVarOrZero != 0)
	{
		...
		for (int i = 0; i < clause.size; i++)
			tldClauseVec->at(i) = clause.begin[i];
...
void SharingManager::digestSharingWithFilter(int *begin, int buflen, const int *filter)
{
	...
	// comment this to disable filter
	if (_params.enableClauseFiltering())
	{
		if (filter != nullptr)
		{
		...
		printf("CLAUSE FILTERING DISABLE");
	}
	...
	while (clause.begin != nullptr)
	{
		...
		// set to 0 to turn off filtering
		...
		for (size_t i = 0; i < importingSolvers.size(); i++)
		{
			...
			else
			{
				...
				if (clause.size == 1)
					unitLists[i].push_front(clause.begin[0]);
				else if (clause.size == 2)
					binaryLists[i].emplace_front(clause.begin[0], clause.begin[1]);
				else
				{
					std::vector<int> clauseVec((explicitLbds ? 1 : 0) + clause.size);
					size_t idx = 0;
					if (explicitLbds)
						clauseVec[idx++] = clause.lbd;
					for (size_t k = 0; k < clause.size; k++)
						clauseVec[idx++] = clause.begin[k];
